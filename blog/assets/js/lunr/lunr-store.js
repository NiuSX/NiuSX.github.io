var store = [{
        ...
        "title": "Pandoc 文档转换工具",
        "excerpt":"pandoc 介绍 Pandoc 是一个“万能文档转换器”，支持在 Markdown、Word (.docx)、PDF、LaTeX、HTML、EPUB 等数十种格式之间互相转换 安装 Pandoc 官网进行下载安装 typora markdown处理器也支持内置Pandoc 使用 查看支持的格式转换 1 2 3 4 5 # 查看所有支持的输入格式 pandoc --list-input-formats # 查看所有支持的输出格式 pandoc --list-output-formats 格式转换 1 pandoc \"原文档名字.源格式\" -o \"目标文档名字.目标格式\" 提取图片到指定文件夹 1 2 # 提取图片到 images 文件夹 pandoc \"测试文档.docx\" -o 测试文档.md --extract-media=images 格式转换速查表 输入格式 输出格式 命令示例 .docx...","categories": ["工具"],
        "tags": [],
