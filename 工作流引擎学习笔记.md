...
> 目标：为“自研一套用于各种任务编排的工作流引擎”建立完整的技术知识地图、架构选型依据和渐进式实现路线。
>
> 范围：本文聚焦机器可执行的任务编排，不把“角色审批、会签、人工节点、组织权限流转”作为核心问题；但会说明未来如何扩展到人工任务。
>
> 更新时间：2026-09-22
>
> 阅读方式：先读第 1～6 章建立语义，再读第 7～13 章做架构设计，最后按第 14～16 章落地和验证。
...
> 一个面向异步、长时间运行和多步骤任务的可靠执行平台。用户提交版本化的工作流定义，平台创建工作流实例，由调度器将可执行任务投递给执行器，执行结果驱动状态推进，并提供重试、超时、定时、取消、查询和审计能力。
...
- 复杂规则引擎和低代码表单；
- 跨数据库的强一致分布式事务；
- 自动替用户推断业务流程的 AI Agent。

但需要预留扩展点：人工任务可以被建模成一种“等待外部完成信号的任务”；规则可以被建模成条件节点或独立决策服务；AI Agent 可以作为受约束的任务执行器，而不是直接拥有任意修改工作流状态的权限。

### 1.3 先回答 12 个需求问题

在设计前写一页需求决策记录，至少回答：

1. 工作流通常运行几秒、几小时，还是几个月？
2. 单个实例最多包含多少节点和分支？
3. 任务是本地函数、容器、HTTP、消息，还是用户提供的插件？
4. 任务失败后默认重试，还是必须由业务决定？
5. 是否要求进程重启后从原位置继续？
6. 是否要求同一个工作流实例不能重复推进？
7. 是否存在两个任务同时写同一业务资源的竞争？
8. 是否需要暂停、恢复、取消、跳过和人工修复？
9. 定时器数量和最远触发时间是多少？
10. 需要至少一次投递、至多一次投递，还是业务上的有效一次？
11. 需要多租户隔离、配额、数据加密和审计到什么程度？
12. 目标是内部平台、通用 SaaS，还是某个垂直业务的专用引擎？

这些问题比“选哪个框架”更重要，因为工作流引擎的复杂度主要由持续时间、失败语义、并发规模和任务边界决定。
...
> 资料阅读建议：先看规范定义，再看产品架构，最后看源码和故障案例。产品文档告诉你“怎么用”，论文和实现细节才更容易回答“为什么这样设计、失败时会怎样”。
